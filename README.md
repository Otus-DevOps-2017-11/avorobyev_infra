...
Выполнил с некоторыми изменениями.

- Можно избежать добавления тегов в каждый элемент файла тасков, пользуясь особенностью импорта: конструкции tags и when наследуются из контекста:

```yaml
- import_tasks: install_app.yml
  tags: install #всем таскам будет присвоен тег install
```
- Редактируя плейбук molecule в который раз смутила необходимость принудительной вставки become: true в контекст вызова роли. Получаем недостаточную абстакцию/инкапсуляцию: - при вызове приходится помнить о специфике некоторых задач роли.  Почему бы сразу не определять этот флаг в задачах, требующих повышения привилегий?

- Добавил в тест установки БД проверку доступности порта:
```python
...
```
... заодно узнал про pytest fixtures. Кстати, File помечена как deprecated, предлагается использовать вызов host.file.

- Задачу с провиженингом в packer думал решить, посмотрев опции ansible провиженера. Но подходящих параметров не нашлось. Странно, так как они есть для ansible_local. Посмотрел gist Николая, но решил сделать другим способом. На роль можно ссылаться не только по имени, но и указывая путь в вызовах include/import. Например, так:

```yaml
#playbooks/packer_app_yaml
  - name: install ruby components
    import_role:
      name: ../roles/app
```
...
В моем случае пришлось разбираться, почему nginx работает ))) Оказалось, я немного сжульничал в прошлом ДЗ и положил конфиг nginx в папку group_vars рядом с плейбуком. У честных людей конфига не было, им пришлось бы ссылаться на свой inventory через опции ansible провиженера или добалять конфиг в Vagrantfile как ruby hash.

Полагаю, что это так. Проверить не могу, так как на работе windows, из под которой без костылей не работает ansible, а на linux vm не работает vagrant с virtualbox провайдером: virtualbox в virtualbox у меня не заработал (я б очень удивился, если заработал)) ).
