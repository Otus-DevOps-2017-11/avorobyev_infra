...
### 1. Интеграция с GCP
Расширил решение динамического инвентаря из предыдущего ДЗ. В исходном файле для хостов через атрибут gcp_name задается идентификатор хоста в gcp. При запуске в выходной конейнер хоста добавляются данных из gcp в поле gcp_data.
...
## PS.нытье
В общем, ansible представилась неким копромиссом между dev и ops, от которого не выиигрывают ни те, ни другие. Для dev  ограничен и ненатурален, для ops сложен и неинтуитивен. Может я что то не понял, но, дизайн, предлагающий хранение экземпляра сущности в разных местах файловой системы, разрушает целостное понимание структуры сайта и заставляет держать в голове кучу утилитарных данных. Лучше бы система представляла модель и API работы с ней на каком либо распространенном ЯП.
